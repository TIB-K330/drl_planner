...
class reciprocal_vel_obs:
    ...
    def vel_select(self, robot_state, vo_outside, vo_inside, nei_state_list, obs_cir_list, obs_line_list, vo_list):
        ...
        if len(vo_outside) != 0:
            ...
            temp = min(vo_inside, key=lambda v: self.penalty(v, vel_des, robot_state, nei_state_list, obs_cir_list,
                                                             obs_line_list, 1))
            return temp
    ...
    def between_vector(line_left_vector, line_right_vector, line_vector):
        ...
        if reciprocal_vel_obs.cross_product(line_left_vector, line_vector) <= 0 and \
                reciprocal_vel_obs.cross_product(line_right_vector, line_vector) >= 0:
            return True
        else:
            return False
    ...
    def between_angle(line_left_ori, line_right_ori, line_ori):
        ...
        if reciprocal_vel_obs.wraptopi(line_ori - line_left_ori) <= 0 and \
                reciprocal_vel_obs.wraptopi(line_ori - line_right_ori) >= 0:
            return True
        else:
            return False
